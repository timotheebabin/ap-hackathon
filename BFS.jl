...
et les valeurs sont un tuple comportant leur distance à l'arrivée,leur prédécesseur dans le plus court chemin entre la position et l'arrivée et la position du zéro dans la position.
"""
function bfs(goal)
    #Initialisation de la carte des distances sous forme de dictionnaire.
    map = Dict{Vector{Int}, Tuple{Int, Vector{Int}, Int}}() 
    #chaque valeur du dictionnaire est un tuple représentant la distance à l'arrivée et le prédécesseur dans le chemin
    map[goal]  = (0, Int[], 9) #le sommet de départ n'a pas de prédécesseur
...
@test trouve_chemin(map, [1,2,3,4,5,0,7,8,6]) == ([[1,2,3,4,5,0,7,8,6],[1,2,3,4,5,6,7,8,0]], [6,9])
